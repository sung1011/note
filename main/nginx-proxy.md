...
1. pass (处理content阶段)
   - 缓存命中 [to 11. 发送响应header](_)
2. 根据指令生成发往上游的header和body
3. request_buffering
   - on; 读取完整请求body
   - off; 边收边转发请求body
4. 根据负载均衡策略选择上游服务
...
1. 是否开启cache 并匹配cache_methods
    - 否: 向上游发起请求
2. 根据cache_convert_head进行HEAD->GET方法转换
3. 根据key生成关键字并md5操作
4. 检查cache_pass是否指明不缓存
    - 是: 向上游发起请求
5. 在共享内存中查询缓存是否存在
    - 不存在: [to 9](_)
6. 更新LRU链表及节点计数
7. 是否错误类响应且过期
    - 是: 向上游发起请求
8. 文件存在且使用次数超出cache_min_users
    - 是: 根据cache_background_update生成子请求; 向下游返回缓存的响应
    - 否: 向上游发起请求
9. 共享内存中分配节点
10. 淘汰已过期的缓存, 再次分配
